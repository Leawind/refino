...
- Agent 指南
  - AGENTS.md（本文件）包含 Agent 应当遵循的规则和应知晓的经验
  - AGENTS-*.md 不纳入版本管理
    - AGENTS-LOCAL.md：因与本地开发环境相关而不适合放在 AGENTS.md 的内容，应当阅读
    - AGENTS-NOTES.md：有时效性的调查快照，通常应忽略，仅当用户明确要求时才访问
- 设计文档
  - 本项目规模较小（<30k LOC），即使将设计文档全量注入模型上下文也可以接受
  - 本项目的设计文档位于 docs/design.md 中，各包将自己的设计细节放在包中的 DESIGN.md 中
  - 文档用简体中文编写，标点符号使用中国大陆规范
  - 文档代表目标，允许超前于代码实现，不可滞后
  - 如果执行任务时发现现有文档存在错误、矛盾、过时、缺失等问题而应当修改，但用户没预料到，应向用户请示，不能编写超前于文档的实现或擅自修改文档
- 用户文档
  - packages/ 中的包应有自己的 README，简单介绍包的作用，不罗列易变的实现细节
- 包的 README.md 和 DESIGN.md 中可以引用该包依赖的其他包，但不能引用依赖它的包
...
- 设计语义与代码强耦合的变更应放在同一个提交中，使每个提交点上的仓库自洽（文档不滞后于已入库代码）。例外：用户先行修改了大量设计文档时，可将文档单独先行提交，以保存这批设计成果、避免其在后续实施过程中丢失或造成干扰；此时的实现提交随后落地即可
...
- 后台环境的 rAF 挂起对渲染器不止是"动画冻结"：`GraphRenderer` 首次向 rAF 注册的回调若永不执行，`#raf` 恒为非零，此后所有 `#schedule()` 都被 `if (this.#raf !== 0) return` 跳过——即使数据、场景、uniform、draw call 全部正常，画布 `readPixels` 永远读到空缓冲，依赖 `#admitted` 剔除集的 `pick()` 也全部失效，表现为"应用像砖了一样什么都不画"。在前台真实浏览器中不存在此问题；在节流环境里验证画布时，别信像素与截图，应改用不依赖 rAF 的 DOM 状态（如状态栏计数）区分数据层与渲染层，再用探针包裹 `drawArraysInstanced` 确认渲染层。参见 `packages/ui/src/graph/render/renderer.ts` 的 `#schedule`。
