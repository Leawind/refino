...
- **模型侧 CRG 工具**：14 个短名 MCP 工具（宿主把全名注入工具清单）——查询（`show` / `search` / `ancestors` / `dependents` / `siblings`…）、待审查、写入（经授权边界校验，越界返回结构化升级报告）、授权查询与对话签发。
- **外部变更同步**：仓库外的图修改经降噪合并，在下次工具调用后（回合内）或下一条消息时以增量更新注入会话。
