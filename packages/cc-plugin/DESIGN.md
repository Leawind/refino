...
批准门为对话批准协议：先在对话中呈现完整草案并获用户明确同意是调用 `request_authorization` 的前置义务（工具描述与技能硬规则承载），宿主的 MCP 工具权限面是可选的机械层。core 的 `requestApproval` 默认实现恒返回 `allowed-once`（协议前置而非门内校验），编排者凭据生效时 core 直接拒绝。签发 delta 同时出现在工具结果（即时反馈）与注入队列（持久记录）。
