...
- **左侧边栏**：仅约束节点，顶部有搜索框；可拖动调宽，可折叠成窄条。
- **右侧边栏**：仅前提节点，顶部有搜索框；同样可调宽、可折叠。
- **中间：决策图**（Decision Graph，CRG 的可视化视图）。
  - 显示方向可自由选择，默认从左到右，可选从上到下、从右到左、从下到上。
  - 布局方式可扩展，将来可加入其他布局（如从中心向四周扩散）。
  - 视图右侧有一列浮动按钮：布局方向切换、新建前提、新建约束。
  - 视图内部的具体交互逻辑暂不设计。
- **节点详情浮窗**：选中节点或新建节点时，在决策图区域内弹出浮窗（略小于图区、居中）。
  - 右上角可放大到近乎全屏（与屏幕边界保留距离）。
  - 展示时背景变暗并虚化；点击浮窗外部或按 Esc 关闭。
- **底部状态栏**：约束节点数、前提节点数在左侧，当前选中节点的 ID 居中。
