...
- 用户级授权状态车道（`state.ts`，Node 专属子路径导出 `@refino/harness/state`）：`~/.refino/workspaces/<根目录哈希>.json` 状态文件的读写与生效授权的解析（编排者凭据 → 用户级状态 → 默认物化，三处来源按优先级）。CLI 与 dsh 插件共用，对话签发跨接入形态、跨 resume 一致；浏览器消费方走主入口，不经此子路径
