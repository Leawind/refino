...
- 编排者凭据解析（`state.ts`，Node 专属子路径导出 `@refino/harness/state`）：`REFINO_AUTHORIZATION` 环境变量或显式路径的凭据文件读取与解析，供工具插件在会话边界兑现编排车道（通用接入形态不实现任务界定层，其 CLI 不消费凭据）。对话车道属插件形态自持（会话内签发），不进入本包；浏览器消费方走主入口，不经此子路径
