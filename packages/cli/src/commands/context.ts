 ...
 * the CRG (scale, root constraints, where to go next). The generic form has
 * no authorization semantics (docs/design.md, "通用接入形态"): reads and
 * writes are unguarded beyond grounds validation, so orientation — not
 * permission — is this command's whole job.
 */
    ...
    .action((_opts, cmd: Command) =>
      run(cmd, (opts: GlobalOptions) =>
        withStore(io, opts, (store) => {
          renderOverview(io, opts, store.graph);
          return 0;
        }),
      ),
    );
...
function renderOverview(io: CliIo, opts: GlobalOptions, graph: Graph): void {
