...
- 修改或删除节点后，主动向用户报告 \`refino pending\` 显示的影响面；待审查项的复核由用户完成，不代替用户确认
...
- 修改或删除节点后向用户报告 \`refino pending\` 的影响面：已改节点与下游待审查约束（前提变化规则见 crg.md 1.6），由用户逐一复核。

## 硬规则

- 仅在仓库已有 \`.refino/\` 时使用 refino；为仓库接入 refino 须经用户明确要求。
- 不直接编辑 \`.refino/nodes/\` 下的文件，一切读写经 refino 命令。
- 约束与前提的变更经 Git 审核生效；影响面的复核由用户完成，不代替用户确认。
