...
refino 在\`.refino/\` 目录中维护的约束细化图（CRG），记录项目的决策（约束，constraint）与事实（前提，premise）及其从抽象到具体的细化关系。约束是必须满足的项目决策，可被有授权地继续细化或修改。
...
- 冻结区：本次任务不可修改的节点集合，由若干约束连同其全部祖先构成。冻结区以外是修改空间，可在其中修改约束、新增细化；新增节点属于修改空间。冻结状态在每次节点渲染中以 \`[冻结]\` 标注，未标注即属修改空间。修改导致下游约束失效时，在同一变更中一并修复。
- 锚点不是签发对象：初始注入的锚点由运行时按图规模自动推导（小图全量注入摘要，大图以 \`refino search\` 按需定位）。
- 写入被冻结区拒绝时返回越界报告：向用户报告阻挡约束、受影响下游与建议方案；用户同意调整冻结区时经签发流程重签，或改走边界内方案。
- 前提或约束变化后，运行 \`refino pending [--base <git-ref>]\`（缺省对比 HEAD，即未提交改动）列出已改节点及其下游待审查约束，逐一复核。

## 任务生命周期

1. 开局：运行 \`refino context\` 获取授权上下文（锚点与前提，摘要级；初始上下文不枚举冻结区）。工作区签发可能属于上一个任务：核对 revision 与 signedAt，不属于本任务时经用户确认重签。大图不注入全图摘要：用 \`refino search\` 按需定位任务相关节点。
2. 按需遍历：先读摘要判断相关性再展开。\`refino show <id>...\` 全文；\`refino grounds|ancestors|dependents <id>...\` 沿边追溯；\`refino search <关键词>\` 分页搜索。查询支持批量 id，按 id 分组、允许部分成功。渲染中带 \`[冻结]\` 标注的节点只读；未标注者及未列出的节点均属修改空间。
...
- 授权只签冻结区（锚点由运行时自动推导，不进入签发）；未签发时默认冻结全部根约束及其祖先。
- \`refino auth show\` 查看生效授权；\`refino auth apply --dry-run\` 预演签发（冻结计数、frontier 归约、解冻根约束警告）；人批准后去掉 \`--dry-run\`执行。多 agent 并存时加 \`--expect-revision <n>\`。\`refino auth reset\`恢复默认。
...
- 越界被拒后不得绕过（包括直接改文件、改授权文件）：向用户报告阻挡原因与建议，按用户裁决行事——调整冻结区须经签发流程，或改走边界内方案。
- 约束与前提的变更经 Git 审核生效。
