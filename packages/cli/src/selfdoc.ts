...
- 前提变化后，用 \`git diff --name-only\` 找出被改节点，对每个节点运行 \`refino dependents <id>\` 复查下游约束。

## 任务生命周期

1. 开局：运行 \`refino context\` 获取授权上下文（锚点与前提，摘要级；初始上下文不枚举冻结区）。工作区签发可能属于上一个任务：核对 revision 与 signedAt，不属于本任务时经用户确认重签。大图不带锚点：用 \`refino search\` 定位后经用户确认签发。
